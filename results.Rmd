...
For aim 2, we created 2 graphs: First one is nationwide new covid cases time series plot from Jan 2020 to Dec 2021. Second one is nationwide flu vaccination coverage rate time series plot from Jan 2018 to Dec 2021. 
...
g<-ggplot(new_cases, aes(submission_date, new_covid_case)) + 
  ...
  labs(y="Count", x="Time", title="Nationwide New Covid Cases")+
  scale_color_colorblind()
...
h<-ggplot(coverage_rate, aes(record_date, estimation)) + 
  geom_smooth(method = "loess", span = .1, se = FALSE)+
  scale_x_date(limits = c(as.Date("2018-01-01"),as.Date("2021-12-01")))+
  labs(y="Coverage Rate", x="Time", title = "Nationwide Flu Vaccination Coverage Rate (%)")+
  scale_color_colorblind()

plot_grid(g,h, nrow=2,align = "v")
```

We observed that the vaccine coverage rate changes every year as a cycle. Each cycle records the cumulative data from June of each year to June of next year. We can see in Figure 1 that the number of people infected has basically increased every day since 2020. However, this has no significant impact on the flu vaccine coverage, which means the data changes after 2020 are basically the same as before 2020. Therefore, we believe that the occurrence of Covid-19 does not affect people's choice of whether to vaccinate against flu.
